...
export default function YearlyBreakdown({ participations }: YearlyBreakdownProps) {
  const sortedParticipations = [...participations].sort((a, b) => b.year - a.year)

  return (
    <Card className="border-0 shadow-sm bg-white/80 backdrop-blur-sm">
      <CardHeader className="pb-4">
        <CardTitle className="text-xl font-semibold text-slate-800">Year by Year</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          {sortedParticipations.map((participation) => (
            ...
            >
              <span className={`font-bold text-slate-800 ${jetbrainsMono.className}`}>{participation.year}</span>
